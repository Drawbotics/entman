...
import { denormalize } from '../../src/denormalizr';
...
describe('denormalize', () => {
  ...
  describe('parsing entities and collections', () => {
    ...
    it('should return the original entity', () => {
      ...
      expect(denormalize(article, data.entities, articleSchema)).toEqual(article1);
    });

    it('should work with entities without values', () => {
      const data = getNormalizedEntities();
      const article = data.entities.articles['3'];
      expect(denormalize(article, data.entities, articleSchema)).toEqual(article3);
    });

    it('should work with entities with empty id', () => {
      const data = getNormalizedEntities();
      const article = data.entities.articles['4'];
      expect(denormalize(article, data.entities, articleSchema)).toEqual(article4);
    });

    it('should return the original entity with id as argument', () => {
      const data = getNormalizedEntities();
      expect(denormalize('1', data.entities, articleSchema)).toEqual(article1);
    });
    ...
    it('does not mutate the entities', () => {
      ...
      expect(normalizedEntities).toEqual(data.entities);
    });
  });
  ...
  describe('parsing interdependents objects', () => {
    ...
    it('should handle recursion for interdependency', () => {
      ...
      expect(denormalized.author.articles[0]).toEqual(denormalized);
    });
  });
  ...
  describe('parsing union schemas', () => {
    describe('when a schema', () => {
      ...
      it('should return the original response', () => {
        ...
        expect(denormalized).toEqual(response.unionItems);
      });
    });
    ...
    describe('when defining a relationship', () => {
      ...
      it('should return the original response', () => {
        ...
        expect(denormalized[1]).toEqual(response.groups[1]);
      });
    });
  });
  ...
  describe('parsing nested plain objects', () => {
    ...
    it('should denormalize nested non entity objects', () => {
      ...
      expect(denormalized).toEqual(response.articles);
    });
  });
  ...
  describe('parsing an array of entities and collections', () => {
    ...
    it('should return an array of entities', () => {
      ...
      expect(denormalized).toEqual(expectedArticles);
    });
  ...
  describe('when adding computed properties', () => {
    ...
    it('should add the computed properties to the final entity', () => {
      ...
      expect(result.getDumbComputedProperty).toBeDefined();
      expect(result.getDumbComputedProperty()).toEqual('dumb-computed-property');
    });
    ...
    it('should add the computed properties to the related entities', () => {
      ...
      const article = data.entities.articles['1'];
      const result = denormalize(article, data.entities, articleSchema);
      expect(result.author.getDumbComputedProperty).toBeDefined();
      expect(result.author.getDumbComputedProperty()).toEqual('dumb-computed-property');
      expect(result.collections.every((c) => c.hasOwnProperty('getDumbComputedProperty'))).toBe(true);
      expect(result.collections.every((c) => c.getDumbComputedProperty() === 'dumb-computed-property')).toBe(true);
    });
